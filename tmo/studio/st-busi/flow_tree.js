            ...
            case "VML":
                switch (tree.config.linkType) {
                    case "M":
                        ...
                        s.push('" strokecolor="' + tree.config.linkColor + '"><v:fill on="false" /><v:stroke StartArrow="" EndArrow="Classic"/></v:polyline>');
                        break;
                    case "B":
                        s.push('<v:curve from="');
                        s.push(xa + ' ' + ya + '" control1="' + xb + ' ' + yb + '" control2="' + xc + ' ' + yc + '" to="' + xd + ' ' + yd);
                        s.push('" strokecolor="' + tree.config.linkColor + '"><v:fill on="false" /><v:stroke StartArrow="" EndArrow="Classic"/></v:curve>');
                        break;
                }
                break;
        ...
        if (!node._isAncestorCollapsed()) {
            switch (this.render) {
                ...
                case "VML":
                    ...
                    s.push('<span>step(' + node.id + ')</span><br>');
                    if (node.canCollapse) {
...
ECOTree.prototype._lastNodeId = 1

ECOTree.prototype.addChild = function(pnode, dsc, w, h, c, bc, target, meta) {
    var pid = -1;
    if (pnode)
        pid = pnode.id;
    var id = this._lastNodeId;
    var node = this.add(id, pid, dsc, w, h, c, bc, target, meta);
    this._lastNodeId++;
    return node;
}
...
ECOTree.prototype.selectNode = function(nodeid, upd) {
    ...
    if (this.onNodeClick) {
        var dbindex = this.mapIDs[nodeid];
        this.onNodeClick(this.nDatabaseNodes[dbindex]);
    }
    if (upd) this.UpdateTree();
}
...
ECOTree.prototype.getParentNode = function(node) {
    var dbindex = this.mapIDs[node.pid];
