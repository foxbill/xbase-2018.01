...
namespace xbase
{
    public class Crypto
    {
        public static string Encrypt(string unEncryptText)
        {
            byte[] bytes = ASCIIEncoding.ASCII.GetBytes(unEncryptText);
            byte[] crypted = SHA256Managed.Create().ComputeHash(bytes);
