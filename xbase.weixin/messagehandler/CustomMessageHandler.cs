...
namespace xbase.weixin.messagehandle
{
    /// <summary>
    /// 自定义MessageHandler
    ...
    public partial class CustomMessageHandler : MessageHandler<CustomMessageContext>
    {
        ...
        private string agentUrl = WxConfigFile.config().Domain;//这里使用了www.weiweihi.com微信自动托管平台
        private string agentToken = WxConfigFile.config().AgentToken;//Token
        private string xbaseKey = WxConfigFile.config().XBaseKey;//WeiweihiKey专门用于对接www.Weiweihi.com平台，获取方式见：http://www.weiweihi.com/ApiDocuments/Item/25#51

        private string appId = WxConfigFile.config().AppID;
        private string appSecret = WxConfigFile.config().AppSecret;
...
<a href=""http://www.w3ccloud.com/FilterTest/"">点击这里</a>进行客户端约束测试（地址：http://www.w3ccloud.com/FilterTest/），如果在微信外打开将直接返回文字。
或：
<a href=""http://www.w3ccloud.com/FilterTest/Redirect"">点击这里</a>进行客户端约束测试（地址：http://www.w3ccloud.com/FilterTest/Redirect），如果在微信外打开将重定向一次URL。";
            }
...
测试完成后，您可以登陆公众号后台取消授权。",
                    Url = "http://www.w3ccloud.com/OpenOAuth/JumpToMpOAuth"
                });
